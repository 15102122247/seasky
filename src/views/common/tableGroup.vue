  ...
  <div @click="bodyClick">
    <div class="group-block">
      <div class="group-block-item">
        <label class="autoLb">工号</label>
        <el-input v-model="checkPersonInfo.userNo" class="wtAotu"></el-input>
      </div>
      <div class="group-block-item" style="position: relative;">
        <label class="autoLb">部门</label>
        <el-input v-model="deptName" @focus="seeTree" id="treeInfoipt" clearable class="wtAotu"></el-input>
      ...
      <div class="group-block-item">
        <label class="autoLb">姓名</label>
        <el-input v-model="checkPersonInfo.userName" class="wtAotu"></el-input>
      </div>
      <div class="group-block-item">
        <label class="autoLb">类型</label>
        <el-select v-model="checkPersonInfo.userType" placeholder="请选择" style="width:209px">
    ...
    <div class="group-table">
      ...
      >
        ...
        <el-table-column prop="userCode" width="80" label="工号"></el-table-column>
        <el-table-column prop="userName" width="80" label="姓名"></el-table-column>
        <el-table-column prop="phone" label="手机号"></el-table-column>
        <el-table-column prop="email" label="邮箱"></el-table-column>
        <el-table-column prop="deptName" label="部门" width="80"></el-table-column>
      </el-table>
    </div>
    <div class="group-pagination">
      <div class="checkAll">
        <el-checkbox v-model="checkedAll" @change="checkAll" :indeterminate="isIndeterminate">全选</el-checkbox>
      </div>
      <el-pagination
    ...
    <add-info :multipleSelection="multipleSelectionPerson" @sendData="infoHandle"></add-info>
    <div class="addPerson">
...
export default {
  ...
  methods: {
    ...
    clearSelection(){
      this.$refs.multipleTableP.clearSelection();
    },
    // 推送列表全选
    checkAll() {
    ...
    async handleCurrentChangeP(val) {
      ...
      let arr = this.listPersonData.filter(p =>
        this.multipleSelectionPerson.map(q => q.userCode).includes(p.userCode)
      );
      if (arr.length == this.listPersonData.length) {
        this.checkedAll = true;
      } else {
